...
seqOptional (h :. t) =
  bindOpt h (\a ->
  bindOpt (seqOptional t) (\list ->
  Full (a :. list)))

bindOpt :: Optional a -> (a -> Optional b) -> Optional b
bindOpt = flip bindOptional
...
-- Using pattern matching but highly inefficient for large lists
-- reverse Nil =
--   Nil
-- reverse (h :. t) =
--   reverse t ++ (h :. Nil)

-- This makes the above tail recursive
reverse0 ::
  List a -- accumulator
  -> List a
  -> List a
